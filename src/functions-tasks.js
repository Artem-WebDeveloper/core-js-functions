...
function getPolynom(...coefficients) {
  ...
  return function polinom(x) {
    let res = 0;
    for (let i = 0; i < coefficients.length; i += 1) {
      const rate = coefficients.length - i - 1;
      res += coefficients[i] * x ** rate;
    }
    return res;
  };
}
...
function memoize(func) {
  ...
  return function memo() {
    if (cached.has(func)) return cached.get(func);
...
function logger(func, logFunc) {
  return function wrapper(...args) {
    const str = `${func.name}(${args.map((arg) => JSON.stringify(arg)).join(',')})`;
    logFunc(`${str} starts`);
    const res = func(...args);
    logFunc(`${str} ends`);
